...
export function MetricCard({ label, value, sub, trend, className }: MetricCardProps) {
  return (
    <div className={cn("bg-surface border border-border rounded-xl p-4 sm:p-5 shadow-[0_1px_2px_rgba(21,25,24,0.04)]", className)}>
      <p className="text-xs font-medium text-text-secondary tracking-wide">{label}</p>
      <p className="text-2xl sm:text-3xl font-bold text-text-primary tracking-tight mt-2 leading-none">{value}</p>
