  ...
  return (
    <Dialog open={open} onClose={onClose} title={title} description={progress} size="md">
      <div className="px-6 py-5">
        {state === "loading" && <p className="text-sm text-[#A1A1AA] py-6 text-center">Loading patients…</p>}
        ...
        {state === "ready" && total === 0 && (
          <p className="text-sm text-[#71717A] py-6 text-center">No one to contact right now.</p>
        )}
        ...
        {state === "ready" && total > 0 && allDone && (
          <div className="py-8 text-center">
            ...
            <h3 className="text-base font-semibold text-[#09090B]">All patients contacted</h3>
            <p className="text-sm text-[#71717A] mt-1">{contacted} of {total} contacted.</p>
            <button
              type="button"
              onClick={onClose}
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="text-lg font-semibold text-[#09090B] leading-tight">{recipient.name}</h3>
          <p className="text-sm text-[#52525B] mt-0.5">{recipient.subject}</p>
        </div>
        {done && (
      ...
      <p className="mt-2 inline-block text-xs text-[#71717A] bg-[#F4F4F5] rounded px-2 py-0.5">
        {recipient.reason}
      </p>
      ...
      <div className="mt-4">
        <div className="flex items-center justify-between">
          <span className="text-xs font-medium uppercase tracking-wider text-[#A1A1AA]">Message</span>
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={copy}
              className="inline-flex items-center gap-1 text-xs font-medium text-[#71717A] hover:text-[#18181B] transition-colors cursor-pointer"
            >
      ...
      {awaitingConfirm ? (
        <div className="mt-4 rounded-lg border border-[#E4E4E7] bg-[#FAFAFA] p-3">
          <p className="text-sm font-medium text-[#09090B]">Did you send this message?</p>
          <div className="mt-2.5 flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center rounded-lg border border-[#E4E4E7] text-[#52525B] px-3.5 py-2 text-sm font-medium hover:bg-white transition-colors cursor-pointer"
            >
              No, keep pending
            </button>
          </div>
        ...
        <div className="mt-4 flex items-center gap-3">
          {link ? (
            ...
            <span className="text-sm text-[#A1A1AA]">No usable phone number for this patient.</span>
          )}
          <button
            type="button"
            onClick={onSkip}
            className="inline-flex items-center text-sm font-medium text-[#71717A] hover:text-[#18181B] transition-colors cursor-pointer"
          >
            Skip
          </button>
        </div>
      ...
      <div className="mt-6 pt-4 border-t border-[#F4F4F5] flex items-center justify-between">
        <button
          ...
          onClick={onPrev}
          disabled={!canPrev}
          className="inline-flex items-center gap-1 text-sm font-medium text-[#52525B] hover:text-[#09090B] transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
        >
