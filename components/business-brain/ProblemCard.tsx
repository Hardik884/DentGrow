      ...
      <div className="flex-1 min-w-0">
        <div className="px-5 py-4">
          <div className="flex items-start justify-between gap-4">
            <h3 className="text-[15px] font-semibold text-[#09090B] leading-snug">{problem.title}</h3>
            {problem.atStake && (
              <div className="text-right shrink-0">
                <div className="text-lg font-semibold text-[#09090B] tabular-nums leading-none">
                  {problem.atStake}
                </div>
                {problem.atStakeLabel && (
                  <div className="text-[11px] text-[#71717A] mt-0.5">{problem.atStakeLabel}</div>
                )}
              </div>
            )}
          </div>
          <p className="text-sm text-[#52525B] mt-1.5">{problem.summary}</p>
          <p className="text-sm text-[#71717A] mt-2 leading-relaxed">{problem.explanation}</p>
        </div>
        ...
        {(problem.howToFix || problem.whyWeThink) && (
          <>
            ...
            >
              <span className="text-sm text-[#52525B]">{open ? "Show less" : "Show more"}</span>
              <ChevronDown
            ...
            {open && (
              <div className="px-5 pb-5 pt-1 space-y-4 animate-fade-in">
                {problem.howToFix && (
                  <div>
                    <p className="text-xs font-medium text-[#71717A] uppercase tracking-wider mb-1">How to fix this</p>
                    <p className="text-sm text-[#52525B] leading-relaxed">{problem.howToFix}</p>
                  </div>
                )}
                {problem.whyWeThink && (
                  <div>
                    <p className="text-xs font-medium text-[#71717A] uppercase tracking-wider mb-1">Why we think this</p>
                    <p className="text-sm text-[#52525B] leading-relaxed">{problem.whyWeThink}</p>
                  </div>
                )}
              </div>
            )}
